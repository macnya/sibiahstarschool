...
export default function ParentRegisterPage() {
  ...
  useEffect(() => {
    if (!initialLoading && user) {
      router.push('/parent-portal'); // Redirect if already logged in
    }
  ...
  async function onSubmit(values: RegisterFormValues) {
    ...
    if (success) {
      router.push('/parent-portal'); // Firebase auto-signs in, redirect to portal
    }
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Create your account to access the Parent Portal."
      />
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <Card className="max-w-md mx-auto shadow-xl">
          <CardHeader className="text-center">
            <UserPlus className="h-16 w-16 mx-auto text-primary mb-4" />
            <CardTitle className="text-2xl font-bold text-primary">Create Parent Account</CardTitle>
          ...
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="emailRegisterParent">Email</FormLabel>
                      <FormControl>
                        <Input id="emailRegisterParent" type="email" placeholder="your.email@example.com" {...field} icon={<Mail className="h-4 w-4 text-muted-foreground" />} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="passwordRegisterParent">Password</FormLabel>
                      <FormControl>
                        <Input id="passwordRegisterParent" type="password" placeholder="Create a Password" {...field} icon={<Lock className="h-4 w-4 text-muted-foreground" />} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="confirmPasswordRegisterParent">Confirm Password</FormLabel>
                      <FormControl>
                        <Input id="confirmPasswordRegisterParent" type="password" placeholder="Confirm Your Password" {...field} icon={<Lock className="h-4 w-4 text-muted-foreground" />} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <div className="text-center text-sm text-muted-foreground">
              ...
              <Button asChild variant="link" size="sm" className="px-0 text-accent hover:underline">
                <Link href="/parent-portal">Login Here</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
